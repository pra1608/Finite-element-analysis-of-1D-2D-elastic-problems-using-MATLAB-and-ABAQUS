...
%
 format long g
fid =fopen('bar_4_results.txt','w');
disp('Results printed to file : bar_3_results.txt');
%
...
for i = 1:nnd
    if (geom(i,2)==Width/2&&geom(i,1)==0)||(geom(i,2)==Width/2&&geom(i,1)==Length)
        Nodal_loads(i,:) = [0,-40];
    elseif geom(i,2)==Width/2
        Nodal_loads(i,:) = [0,-80];
